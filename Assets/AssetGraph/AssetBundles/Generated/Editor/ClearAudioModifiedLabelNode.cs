...
/// <summary>
/// 清除音频文件被修改后添加的标记，需要放在AudioNode节点前
/// </summary>
[CustomNode("Custom/ClearAudioModifiedLabelNode", 1000)]
public class ClearAudioModifiedLabelNode : Node {
	...
	public override Node Clone(Model.NodeData newData) {
		var newNode = new ClearAudioModifiedLabelNode();
			...
			foreach (var ag in incoming)
			{
				foreach (var group in ag.assetGroups)
				{
					foreach (var asset in group.Value)
					{
						...
						{
							isSaveAssets = true;
							AssetDatabase.SetLabels(AssetDatabase.LoadMainAssetAtPath(path), new string[] { });
						}
					}
				}
			}
