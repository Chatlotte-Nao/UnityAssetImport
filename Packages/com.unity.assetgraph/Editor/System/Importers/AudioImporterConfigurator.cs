...
namespace UnityEngine.AssetGraph
{
    ...
    public class AudioImportSettingsConfigurator : IAssetImporterConfigurator
    {
        ...
        {
            var defaultSetting = reference.defaultSampleSettings;
            defaultSetting.preloadAudioData = reference.defaultSampleSettings.preloadAudioData; 
            target.defaultSampleSettings = defaultSetting;
            ...
            foreach (var g in NodeGUIUtility.SupportedBuildTargetGroups)
            {
                ...
                if (reference.ContainsSampleSettingsOverride(platformName))
                {
                    ...
                    setting.preloadAudioData = reference.GetOverrideSampleSettings(platformName).preloadAudioData; // ✅ 复制 preloadAudioData
        ...
        private bool IsEqual(AudioImporter target, AudioImporter reference)
        {
            ...
            foreach (var g in NodeGUIUtility.SupportedBuildTargetGroups)
            {
                ...
                if (target.ContainsSampleSettingsOverride(platformName) != reference.ContainsSampleSettingsOverride(platformName))
                {
                    return false;
                }
